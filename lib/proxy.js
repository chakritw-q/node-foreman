...
function f(key, j, ports, proc, reqs, portargs, localhost, emitter, ssl) {
  ...
  //Q:20180316: UC Mod {
  var respawnDelay=process.env.FOREMAN_PROXY_RESPAWN_DELAY || 1000;
  function scheduleRespawn(delay) {
    cons.Alert('proxy: Waiting to respawn (' + delay + ' msecs) ...');
    
    setTimeout(function() {
      f(key, j, ports, proc, reqs, portargs, localhost, emitter, ssl);
    }, delay);
  }
  proxy.on('exit', function(code, signal) {
    cons.Alert('proxy: Received exit signal: ' + signal + ' (code: ' + code + ')');
    //emitter.emit('killall', signal);
    scheduleRespawn(respawnDelay);
  });
  proxy.on('SIGTERM', function() {
    cons.Alert('proxy: Received SIGTERM');
    proxy.exit(-1);
  });
  proxy.on('unhandledException', function(err) {
    //var util = require('util');
    cons.Alert('proxy: Unhandled exception: ' + util.inspect(err));
    //emitter.emit('killall', signal);
    proxy.exit(err.code || -1);
  });
  // }
